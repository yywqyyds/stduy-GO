...
func main() {
	...
	if stat, _ := file.Stat(); stat.Size() == 0 {
		...
		file.WriteString("游戏次数 | 游戏开始时间 | 游戏结束时间 | 难度 | 猜测次数 | 猜测数字 | 结果 | 用时 | 正确答案\n")
	}
	...
	for {
		...
		for i := 1; i <= guessTimes; i++ {
			...
			attempt = i
			guesses = append(guesses, fmt.Sprintf("%d", guessNumber)) //记录猜测数字
			...
			if guessNumber > number {
				...
				flag = true
				record = fmt.Sprintf("%d | %s | [%s] | 难度%d | %d次 | [%s] | 成功 | %.1f秒 | 答案:%d\n",
					playCount, startTime.Format("2006-01-02 15:04:05"), time.Now().Format("2006-01-02 15:04:05"), difficulty, i, strings.Join(guesses, ","), guessNumberTime.Seconds(), number)
				break
			}
		}
		...
		if !flag {
			...
			record = fmt.Sprintf("%d | %s | [%s] | 难度%d | %d次 | [%s] | 失败 | %.1f秒 | 正确答案:%d\n",
				playCount, startTime.Format("2006-01-02 15:04:05"), time.Now().Format("2006-01-02 15:04:05"), difficulty, attempt, strings.Join(guesses, ","), guessNumberTime.Seconds(), number)
		}
